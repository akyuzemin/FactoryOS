...
<main class="main-content">
    ...
    <!-- HAT BAZLI SPESİFİK ENERJİ TÜKETİMİ (SEC) TABLOSU -->
    <?php if (!empty($lineSecData['lines'])): ?>
    <div class="saas-card" style="margin-bottom: 24px;">
        <div class="saas-card-header">
            <div>
                <h3 class="saas-card-title">🏭 Üretim Hatları Spesifik Enerji Tüketimi (SEC) &amp; Panel Başı Tüketim</h3>
                <p class="saas-card-subtitle">
                    Hat sayaç okumaları ve üretim logları üzerinden hesaplanan ortalama spesifik enerji tüketimi (Hat Tüketimi / Üretilen Panel Adedi)
                </p>
            </div>
            <span style="font-size: 11.5px; font-weight: 700; color: #475569; background: #f1f5f9; padding: 4px 10px; border-radius: 6px;">
                Tesis Ortalama SEC: <?= number_format((float)($lineSecData['plant_sec_kwh_per_panel'] ?? 0), 2, ',', '.') ?> kWh/panel
            </span>
        </div>
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 13px; text-align: left;">
                <thead>
                    <tr style="background: #f8fafc; border-bottom: 1px solid #e2e8f0;">
                        <th style="padding: 10px 14px; font-weight: 700; color: #475569;">Üretim Hattı</th>
                        <th style="padding: 10px 14px; font-weight: 700; color: #475569; text-align: right;">Toplam Tüketim (kWh)</th>
                        <th style="padding: 10px 14px; font-weight: 700; color: #475569; text-align: right;">Üretilen Panel</th>
                        <th style="padding: 10px 14px; font-weight: 700; color: #0f172a; text-align: right;">Ortalama Enerji / Panel (SEC)</th>
                        <th style="padding: 10px 14px; font-weight: 700; color: #0f172a; text-align: right;">Ortalama Enerji Maliyeti / Panel</th>
                        <th style="padding: 10px 14px; font-weight: 700; color: #475569; text-align: center;">Durum</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($lineSecData['lines'] as $line): ?>
                        <tr style="border-bottom: 1px solid #f1f5f9;">
                            <td style="padding: 12px 14px; font-weight: 700; color: #0f172a;">
                                <?= htmlspecialchars($line['line_name']) ?>
                                <span style="font-size: 11px; font-weight: 400; color: #64748b; display: block; font-family: monospace;"><?= htmlspecialchars($line['line_code']) ?></span>
                            </td>
                            <td style="padding: 12px 14px; text-align: right; font-weight: 600; color: #334155;">
                                <?= number_format((float)$line['total_kwh'], 1, ',', '.') ?> kWh
                            </td>
                            <td style="padding: 12px 14px; text-align: right; font-weight: 700; color: #0f172a;">
                                <?= number_format((int)$line['produced_panels'], 0, ',', '.') ?> Adet
                            </td>
                            <td style="padding: 12px 14px; text-align: right; font-weight: 800; color: #2563eb;">
                                <?= number_format((float)$line['sec_kwh_per_panel'], 2, ',', '.') ?> <small>kWh/panel</small>
                            </td>
                            <td style="padding: 12px 14px; text-align: right; font-weight: 800; color: #16a34a;">
                                ₺<?= number_format((float)$line['unit_energy_cost_tl_per_panel'], 2, ',', '.') ?> <small>/panel</small>
                            </td>
                            <td style="padding: 12px 14px; text-align: center;">
                                <?php if ($line['status_badge'] === 'WARNING'): ?>
                                    <span style="padding: 3px 8px; border-radius: 12px; font-size: 11px; font-weight: 700; background: #fef3c7; color: #92400e; border: 1px solid #fcd34d;">
                                        ⚠️ Yüksek SEC
                                    </span>
                                <?php else: ?>
                                    <span style="padding: 3px 8px; border-radius: 12px; font-size: 11px; font-weight: 700; background: #dcfce7; color: #166534; border: 1px solid #86efac;">
                                        🟢 Normal
                                    </span>
                                <?php endif; ?>
                            </td>
