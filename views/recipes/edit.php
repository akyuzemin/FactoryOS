...
<main class="main-content">
    ...
    <header class="page-header">
        <div>
            ...
            <h1>✏️ Reçeteyi Düzenle: <?= htmlspecialchars($recipe['name']) ?></h1>
            <p class="page-description">Reçete kodunu, baz miktarını ve hammadde tüketim kalemlerini güncelleyin.</p>
        </div>
    ...
    <form method="POST" action="/stok-takip/public/recipes/edit?id=<?= (int)$recipe['id'] ?>
            <?= CsrfService::tokenField() ?>" id="recipe-form">
        ...
        <div class="card" style="padding: 24px; margin-bottom: 24px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 18px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="code" name="code" class="form-input" value="<?= htmlspecialchars($formData['code'] ?? '') ?>" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-input" value="<?= htmlspecialchars($formData['name'] ?? '') ?>" required>
                </div>

                <!-- Baz Üretim Miktarı -->
                <div class="form-group">
                    <label for="base_quantity">Baz Üretim Miktarı <span style="color:#ef4444;">*</span></label>
                    <input type="number" step="0.001" min="0.001" id="base_quantity" name="base_quantity" class="form-input" value="<?= htmlspecialchars((string)($formData['base_quantity'] ?? '1.000')) ?>" required>
                    <small style="color: var(--text-secondary); font-size: 11px;">Aşağıdaki malzeme oranlarının baz alındığı bitmiş ürün adedi</small>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="is_active" name="is_active" class="form-select">
                        <option value="1" <?= ($formData['is_active'] ?? 1) == 1 ? 'selected' : '' ?>>Aktif (Kullanımda)</option>
                        <option value="0" <?= ($formData['is_active'] ?? 1) == 0 ? 'selected' : '' ?>>Pasif (Kullanım Dışı)</option>
                    </select>
                </div>
                ...
                <div class="form-group" style="grid-column: 1 / -1;">
                    ...
                    <textarea id="description" name="description" class="form-textarea" rows="2"><?= htmlspecialchars($formData['description'] ?? '') ?></textarea>
                </div>
